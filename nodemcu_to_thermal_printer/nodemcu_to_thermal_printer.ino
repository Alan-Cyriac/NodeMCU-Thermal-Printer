...
void handlePrint(){
  if (server.hasArg("data")== false){ //Check if content received
    ...
    for(int i=0;i<size;i++){
      adalogo_data[i] = (uint8_t)doc["data"][i];
    }
    if( printer.hasPaper() ) {
...
void handleConfig(){
  if (server.hasArg("data")== false){ //Check if content received
    ...
    int heatTime = doc["heatTime"];
    int heatInterval = doc["heatInterval"];
    char printDensity = doc["printDensity"];
    char printBreakTime = doc["printBreakTime"];
...
void setup(void){
  ...
  Serial.println( "DESKTOP-Abhijith M" );
  
// Sending SSID and Password...
  WiFi.begin("DESKTOP-Abhijith M","initial#024");
  Serial.println();
